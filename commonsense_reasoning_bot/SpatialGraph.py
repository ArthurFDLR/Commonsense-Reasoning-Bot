...
class ObjectSet:
    ...
    def addObject(
        ...
        width: float = 0.0,
        theta: float = 0.0,
        objType: FurnitureType = None,
    ):
        if not self.isObject(name):
            if not objType:
                self._objects[name] = Furniture(
                    heigth=heigth, width=width, x=x, y=y, theta=theta
                )
            if objType == FurnitureType.Table:
                self._objects[name] = Table(
                    heigth=heigth, width=width, x=x, y=y, theta=theta
                )
            if objType == FurnitureType.Chair:
                self._objects[name] = Chair(
                    heigth=heigth, width=width, x=x, y=y, theta=theta
                )
            return True
        else:
    ...
    def getCoordinate(self, name: str):
        if self.isObject(name):
            return [
                self._objects[name].x,
                self._objects[name].y,
                self._objects[name].theta,
            ]
        else:
            return None
...
def MyScene() -> SpatialGraph:
    ...
    objects.addObject(
        name="table6", objType=FurnitureType.Table, x=2.5, y=2.2, heigth=0.8, width=1.6
    )
    objects.addObject(
        name="chair1t6",
        objType=FurnitureType.Chair,
...
class ClickablePlotWidget(pg.PlotWidget):
    ...
    def __init__(
        ...
        graph: SpatialGraph,
        objects: ObjectSet,
        positionClicked: pyqtSignal,
        addClient_signal: pyqtSignal,
        removeClient_signal: pyqtSignal,
        newPositionPepper_signal: pyqtSignal,
        tableCallBill_signal: pyqtSignal,
    ):
    ...
    def mouse_clicked(self, mouseClickEvent):
        ...
        # Test if an item is clicked (and not background)
        if x % 1.0 != 0.0 and y % 1.0 != 0.0:
            ...
            obj_is_occupied = self.objects.isOccupied(self.lastObjClicked)
            status_str = ": occupied" if obj_is_occupied else ": free"
            self.setMenuTitle(self.lastObjClicked + status_str)

            if self.objects.isChair(self.lastObjClicked):
                self.addClientAction.setText("Add client")
                self.addClientAction.setEnabled(not obj_is_occupied)
                self.removeClientAction.setEnabled(obj_is_occupied)
    ...
    def getMenu(self):
        ...
        if self.menu is None:
            ...
            self.addClientAction.triggered.connect(
                lambda: self.addClient_signal.emit(self.lastObjClicked)
            )
            ...
            self.removeClientAction.triggered.connect(
                lambda: self.removeClient_signal.emit(self.lastObjClicked)
            )
            ...
            self.sendPepperAction.triggered.connect(
                lambda: self.newPositionPepper_signal.emit(self.lastObjClicked, 0.0)
            )
...
class GraphPlotWidget(Qtw.QWidget):
    ...
    def __init__(
        ...
        self.graphWidget = ClickablePlotWidget(
            ...
            self.newPositionPepper_signal,
            tableCallBill_signal,
        )
    ...
    def resetPlot(self):
        ...
        for o in self.objects.getObjects(Table):  # Plot tables
            ...
            self.graphWidget.plot(
                [oPos[0]], [oPos[1]], symbol="s", symbolSize=50, symbolBrush=("r")
            )

        for c in self.objects.getObjects(Chair):  # Plot chairs
            cPos = self.objects.getCoordinate(c)
            self.graphWidget.plot(
                [cPos[0]], [cPos[1]], symbol="s", symbolSize=20, symbolBrush=("b")
            )
